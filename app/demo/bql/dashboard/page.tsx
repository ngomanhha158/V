import {
  Card, CardHead, Chip, Hop, NhanNhom, PageHead, Stat, cx, ngayVN, soVN, vnd, vndGon,
} from '@/components/ui'
import { BangThang, ChuThichThu, CotThu, DuongSLA, type ThangKPI } from '@/components/chart'
import { KY, laKy, type KyKey } from '@/lib/ky'
import { DASHBOARD, DASHBOARD_THANG, DU_AN } from '@/lib/demo/data'
...
export default async function DemoDashboard({
  ...
  // Bản demo dùng MỘT bộ số cho mọi khoảng thời gian. Sinh số giả khác nhau
  // theo từng kỳ chỉ tạo cảm giác nó biết tính toán, trong khi nó không tính gì.
  const project = { id: 'demo', name: DU_AN.ten }
  const t = DASHBOARD
  const thang: ThangKPI[] = DASHBOARD_THANG
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2">
        {(Object.keys(KY) as KyKey[]).map((k) => (
          <Chip key={k} href={`/demo/bql/dashboard?ky=${k}`} active={ky === k}>{KY[k]}</Chip>
        ))}
      </div>

      {/* ── Bốn số BQT mang đi họp — xem ghi chú ở bản thật ─────────────── */}
