...
export default async function DemoGoLive() {
  const rows = [SAN_SANG]
  const d = rows[0]
  const bank = null as { bin: string; accountNumber: string } | null
  const host = 'vbuilding.example.vn'
  ...
  const coServiceKey = true
  const coWebhook = true
  ...
  const mucs: Muc[] = [
    { ten: 'Đã nhập tòa và căn hộ', xong: d.so_can > 0, batBuoc: true,
      ...
      lam: { nhan: 'Nhập từ Excel', href: '/demo/bql/import' } },
    ...
    { ten: 'Đã đăng nội quy / sổ tay', xong: d.so_noi_quy > 0, batBuoc: false,
      ...
      lam: { nhan: 'Soạn sổ tay', href: '/demo/bql/so-tay' } },
    ...
    { ten: 'Đã phát hành hóa đơn kỳ này', xong: d.so_hoa_don_da_phat > 0, batBuoc: false,
      ...
      lam: { nhan: 'Sang màn hóa đơn', href: '/demo/bql/billing' } },
    ...
    { ten: 'Không còn yêu cầu chủ hộ nào chờ duyệt', xong: d.so_cho_duyet === 0, batBuoc: false,
      ...
      lam: { nhan: 'Duyệt ngay', href: '/demo/bql/duyet-chu-ho' } },
  ]
  ...
  return (
    <div className="space-y-5">
      <PageHead
        ...
        sub={`${DU_AN.ten} · kiểm trước khi dán poster và mở cho cư dân`}
        actions={
          conThieu.length === 0
